...
| TS-006 | Local Dev Mode + ngrok | TS-005 completed; `pizzaz_server_node/.env` contains `ENVIRONMENT=local` and `PORT=8123` | 1. From repo root run `pnpm run dev`. 2. Start Node server (TS-004). 3. Launch tunnel: `ngrok.exe http 8123`. 4. Configure ChatGPT connector with `<ngrok-url>/mcp` and invoke the pizza map tool after editing `src/pizzaz/index.jsx`. 5. Record result (PASS if widgets stream over ngrok and reflect edits). | Server logs `dev-*` version; widget reloads without hash suffix, reflecting source change via ChatGPT session. | PASS
